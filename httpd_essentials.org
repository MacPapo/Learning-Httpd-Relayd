#+title: Httpd Essentials
#+author: Jacopo Costantini

* Privilege Separation

   A web server must perform certain tasks with root-level privileges.
...
#+end_quote
   ...
   OpenBSD uses /privilege separation/ to divide tasks that require *root*
...
** Process:
   ...
   1. When you start httpd, you'll get a *single process* running as *root*.
      ...
      This process binds to *ports, opens log files, reads private keys,
      and handle all the other privileged tasks*.

   2. A *second process* will lose its privileges for everything but handling logs.

   3. You also get enterily separate processes that handle *client requests*.
   ...
   When an *unprivileged process* needs privileged information,
   it requests it from the parent.
   The parent determines if the child gets that information or not.

   That's no reason for the logger to have private keys,
   so it can never get that information.


* Httpd

  The httpd daemon software httpd normally runs whitout any arguments and is started at boot or rcctl.
...
** Validating the configuration (/etc/httpd.conf)
   ...
   If you are testing an alternative configuration file,
   specify that file with -f
...
* /etc/httpd.conf
   ...
   The httpd configuration file resambles other OpenBSD daemons, with a keyword and value syntax.
   Curly braces mark subsections, such as settings for individual virtual hosts.

   OpenBSD doesn't install a configuration file, but you can find a
   detailed sample configuration in //etc/examples/httpd.conf/
   ...
   This configuration defines a *server name*, attaches the *httpd daemon* to an *IP address and port*, and *points the server to a document root directory*
...
* The Httpd Chroot
    ...
    You will find several subdirectories in the *chroot*
...
#+end_quote
    ...
    It's possible that operating system upgrades might change the contents of //var/www/. You might want to change the location of your chroot, so that you can precisely control what goes on in your site.
...
** Virtual Servers
   ...
   You can add virtual servers until the traffic load slows your server unacceptably.
...
#+end_quote
    ...
    Many automation systems expect exactly this configuration.
...
** Networking
   ...
   When give an interface name or group name, httpd attaches the primary /IPv4 address/ of that interface.
...
** Indexes
   ...
   When a client hits a web site, the server looks for an index document to serve as the front page. The traditional index file is index.html.
   ...
   The site generate index for the user, and defaults to using an index file of index.htm
...
** Locations
   ...
   You could use symlinks to attach the /cgi-bin/ directory to a web site, or use a location directive in //httpd.conf/ to accomplish the same thing.
...
* MIME Types
   ...
   Use the /types/ statement to set /MIME/ types in /httpd.conf/.
...
* Authentication
   ...
   OpenBSD's htpasswd only support bcrypt, httpd only supports bcrypt passwords.
